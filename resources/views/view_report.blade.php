		...
		<div class="card mb-5 mb-xl-10">
			<div class="card-body pt-9 pb-0">
				...
				<div class="card-header cursor-pointer">
					<!--begin::Card title-->
					@php
						$month = Carbon\Carbon::now()->month($month)->format('F');
					@endphp
					<div class="card-title m-0">
						<h3 class="fw-bolder m-0">Payout List({{$month}}-{{$year}})</h3>
					</div>
					@if(count($payouts) != 0)
						<a href="{{route('report-excel-csv-file', ['slug' => 'xlsx','month' => $month,'year' => $year])}}" class="btn btn-sm btn-light btn-active-primary align-self-center">Export data</a>
					@endif
