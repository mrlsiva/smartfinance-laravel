...
class smartfinanceController extends Controller
{
    ...
    public function payout_list(Request $request) 
    {
        ...
        $users = DB::table('smartfinance_payments')->join('smartfinances','smartfinance_payments.smartfinance_id','=','smartfinances.id')->whereMonth('smartfinance_payments.payment_date',$month)->whereYear('smartfinance_payments.payment_date', $year)->groupBy('smartfinances.user_id')->select('smartfinances.user_id as user_id')->get();
        $amount = 0;

        foreach($users as $user){

            $payments = SmartfinancePayment::join('smartfinances','smartfinance_payments.smartfinance_id','=','smartfinances.id')->whereMonth('smartfinance_payments.payment_date',$month)->whereYear('smartfinance_payments.payment_date', $year)->where('smartfinances.user_id',$user->user_id)->select('smartfinance_payments.*')->get();
